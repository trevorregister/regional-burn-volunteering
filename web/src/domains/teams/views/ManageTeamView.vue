    ...
    <v-col>
      <v-row>
        ...
        <v-col cols="2" class="d-flex align-center pa-0">
          <action-button v-if="!showCol" @click="toggleCol" :label="'Create Shifts'" />
        </v-col>
      </v-row>
      <v-row>
        <h1>{{ team.name }}</h1>
      ...
      <v-row>
        <v-col>
          <shift-table @shift-action="shiftAction" :shifts="shifts" :userShifts="[]" />
        </v-col>
        <v-col v-if="showCol" cols="4">
          <create-shift-form @cancel="toggleCol" @shifts-created="load" />
        </v-col>
      </v-row>
    </v-col>
  </v-row>
