<template>
  <v-form class="add-shift-form">
    <v-text-field label="Name" v-model="shift.name" variant="underlined" @blur="floorValues" />
    <v-text-field
      label="Description"
    ...
    >
      <v-tooltip activator="parent"> Rounds to nearest hour </v-tooltip>
    </v-text-field>
    <v-text-field
      ...
      @blur="floorValues"
    >
      <v-tooltip activator="parent"> Whole values only </v-tooltip>
    </v-text-field>
    <v-text-field
      ...
      @blur="floorValues"
    >
      <v-tooltip activator="parent"> Create multiple back-to-back shifts </v-tooltip>
    </v-text-field>
    <v-row>
      <v-col class="d-flex justify-center pa-1">
        <action-button @click="toggleModal" :label="'Create'" />
      </v-col>
      <v-col class="d-flex justify-center pa-1">
        <warning-button @click="cancel" :label="'Cancel'" />
      </v-col>
    </v-row>
  </v-form>
  <v-container>
    <v-overlay v-model="showModal" scroll-strategy="none" class="align-center justify-center">
      <v-sheet elevation="5" class="rounded-lg">
        ...
        <v-table>
          ...
          <tbody>
            <tr v-for="shift in shifts" :key="shift">
              ...
              <td>{{ (shift.end - shift.start) / 3600000 }}</td>
              <!-- converts to hours -->
        ...
        <v-row justify="space-around" class="py-2">
          <action-button @click="create" :label="'Confirm'" />
          <warning-button @click="toggleModal" :label="'Cancel'" />
        </v-row>
      </v-sheet>
    </v-overlay>
  </v-container>
